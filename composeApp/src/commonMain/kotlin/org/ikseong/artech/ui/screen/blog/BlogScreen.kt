...
import org.ikseong.artech.ui.component.BlogInfoCard
...
fun BlogScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.uiEffect.collect { effect ->
            when (effect) {
                BlogUiEffect.ScrollToTop -> listState.scrollToItem(0)
            }
        }
    }

    // stickyHeader가 상단에 고정되었는지 감지
            ...
            uiState.error != null && uiState.articles.isEmpty() -> {
                ...
                Box(modifier = Modifier.fillMaxSize()) {
                    LazyColumn(
                        ...
                        // BlogInfoCard (스크롤됨)
                        uiState.blogMeta?.let { meta ->
                            item(key = "blog_info") {
                                BlogInfoCard(
                                    blogMeta = meta,
                                    articleCount = uiState.totalArticleCount,
